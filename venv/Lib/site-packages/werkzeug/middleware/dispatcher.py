...
This example dispatches to an API sweater under "/api", an admin sweater
under "/admin", and an sweater that serves frontend files for all other
requests::

    sweater = DispatcherMiddleware(serve_frontend, {
        '/api': api_app,
        '/admin': admin_app,
    })
